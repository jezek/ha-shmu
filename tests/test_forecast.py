...
class TestForecastHelperContract(unittest.TestCase):
    ...
    def test_rows_as_daily_forecast_excludes_initial_past_day_with_history(self):
        ...
        rows = forecast.parse_helper_forecast(
            {
                "model_run_time": "2026-07-25T18:00:00Z",
                "source_url": "https://example.test/aladin.json",
                "source_run_id": "run",
                "rows": [
                    {
        ...
        history = {day + timedelta(hours=hour): 5 + hour for hour in range(18)}
    ...
    def test_20260804_0600_complete_run_has_daily_forecast(self):
        model_run_time = datetime(2026, 8, 4, 6, tzinfo=timezone.utc)
        rows = forecast.parse_helper_forecast(
            {
                "model_run_time": model_run_time.isoformat(),
                "source_url": "https://opendata.shmu.sk/meteorology/weather/nwp/aladin/sk/4.5km/20260804/0600",
                "source_run_id": "aladin-sk-4.5km-20260804-0600",
                "rows": [
                    {
                        "valid_time": (
                            model_run_time + timedelta(hours=lead)
